...
class DeepLChannel implements ITranslateInterface {
  ...
  apiTranslate(info): void {
    ...
    GlobalWin.mainWin.webContents.send('agent-api-translate', TranslateServiceEnum.DEEP_L, info, false)
  }

  apiTranslateCheck(info): void {
    log.info('[DeepL翻译事件] - 请求报文 : ', paramsFilter(info))
    GlobalWin.mainWin.webContents.send('agent-api-translate', TranslateServiceEnum.DEEP_L, info, true)
  }
  ...
  static apiTranslateCallback(status, data): void {
    if (status) {
      ...
      GlobalWin.mainWin.webContents.send('deepl-api-translate-callback-event',
        R.okT(data['translations'].map((translation) => translation.text))
      )
    } else {
      GlobalWin.mainWin.webContents.send('deepl-api-translate-callback-event', R.okT(DeepLChannel.commonErrorExpand(data)))
    }
  }
  ...
  static apiTranslateCheckCallback(status, data, info): void {
    ...
    if (status) {
      ...
      GlobalWin.setWin.webContents.send('api-check-translate-callback-event', TranslateServiceEnum.DEEP_L, R.okD(responseData))
    } else {
      GlobalWin.setWin.webContents.send('api-check-translate-callback-event', TranslateServiceEnum.DEEP_L, R.errorMD(DeepLChannel.commonErrorExpand(data), responseData))
    }
  }
